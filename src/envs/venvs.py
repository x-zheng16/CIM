...
class SubprocEnvWorker(worker.SubprocEnvWorker):
    def __init__(self, env_fn, share_memory=False, bind_core=False, index=None, pid_bias=0):
        ...
        self.process.start()
        if self.bind_core:
            os.system(f"taskset -p -c {index % os.cpu_count() + pid_bias:d} {self.process.pid} > /dev/null")
...
class SubprocVectorEnv(BaseVectorEnv):
    def __init__(self, env_fns, pid_bias=0, bind_core=False, **kwargs):
        worker_fn = lambda index, env_f: SubprocEnvWorker(env_f, bind_core=bind_core, index=index, pid_bias=pid_bias)
        super().__init__(env_fns, worker_fn, **kwargs)


class ShmemVectorEnv(BaseVectorEnv):
    def __init__(self, env_fns, pid_bias=0, bind_core=False, **kwargs):
        worker_fn = lambda index, env_f: SubprocEnvWorker(
